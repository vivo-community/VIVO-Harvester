...
public class WoWRSSReader {
 
	private static WoWRSSReader instance = null;
	
	private WoWRSSReader() {
	}
	
	public static WoWRSSReader getInstance() {
		if(instance == null) {
			instance = new WoWRSSReader();	
		}
		return instance;
	}
	...
	public void writeNews() {
		try {
			...
			BufferedWriter out = new BufferedWriter(new FileWriter("WoW-RSSDump-" + dateString + ".txt"));
...
//				out.write("content: " + getElementValue(element,"content"));
		...
		WoWRSSReader reader = WoWRSSReader.getInstance();
		reader.writeNews();
		}
}

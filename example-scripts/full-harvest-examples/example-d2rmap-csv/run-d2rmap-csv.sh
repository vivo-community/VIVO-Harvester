...
export HARVEST_NAME=example-d2rmap-csv
...
# clone db (comment this out for csv ingest)
...
harvester-d2rmapfetch -X d2rmap-csv-fetch.config.xml
...
# Execute Score for Roles
harvester-score -X score-roleleaders.config.xml
...
# Execute Match for People and Courses
harvester-match -X match-people-courses.config.xml

#Truncate Score Data model
# Since we are finished with the scoring data for people and courses,
...
harvester-match -X match-roleleaders.config.xml

# Execute ChangeNamespace to get unmatched  into current namespace
# This is where the new people, courses, datetimes, and roles from the harvest are given uris within the namespace of Vivo
...
# Execute ChangeNamespace for Courses
harvester-changenamespace -X changenamespace-courses.config.xml

# Execute ChangeNamespace for Roles
harvester-changenamespace -X changenamespace-roleleaders.config.xml

# Execute ChangeNamespace for Dates
harvester-changenamespace -X changenamespace-dates.config.xml

# Execute ChangeNamespace for DateTimeIntervals
harvester-changenamespace -X changenamespace-dtintervals.config.xml
...
COURSE=`cat data/vivo-additions.rdf.xml | grep 'http://vivoweb.org/ontology/core#Course' | wc -l`
ROLELEADER=`cat data/vivo-additions.rdf.xml | grep 'leaderRoleOf' | wc -l`
echo "Imported $PEOPLE people, $COURSE courses, and $ROLELEADER leader roles"

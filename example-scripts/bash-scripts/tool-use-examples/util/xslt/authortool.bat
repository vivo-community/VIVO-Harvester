...
IF exist data\translated_records (
  rmdir /s /q data\translated_records
)
...
@java %HARVESTER_JAVA_OPTS% -cp %CLASSPATH% org.vivoweb.harvester.translate.XSLTranslator -X xsltranslator.conf.xml

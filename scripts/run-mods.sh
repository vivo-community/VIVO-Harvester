...
# Find Subtractions
$Diff -m $H2MODEL -MmodelName=$PREVHARVESTMODEL -McheckEmpty=$CHECKEMPTY -MdbUrl=${PREVHARVDBURLBASE}${HARVESTER_TASK}/store -s $H2MODEL -ScheckEmpty=$CHECKEMPTY -SdbUrl=$MODELDBURL -SmodelName=$MODELNAME -d $SUBFILE
# Find Additions
$Diff -m $H2MODEL -McheckEmpty=$CHECKEMPTY -MdbUrl=$MODELDBURL -MmodelName=$MODELNAME -s $H2MODEL -ScheckEmpty=$CHECKEMPTY -SmodelName=$PREVHARVESTMODEL -SdbUrl=${PREVHARVDBURLBASE}${HARVESTER_TASK}/store -d $ADDFILE

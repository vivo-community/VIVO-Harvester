...
java -cp lib/d2rmap-V03.jar:bin/ingest-0.7.0.jar:bin/dependency/* org.vivoweb.ingest.fetch.D2RMapFetch -o config/recordHandlers/JDBCXMLRecordHandler.xml -u config/tasks/D2RMapFetchTask.d2r.xml -s person.rdf

# Execute Transfer to transfer rdf into "d2rStaging" JENA model
java -cp bin/ingest-0.7.0.jar:bin/dependency/* org.vivoweb.ingest.transfer.Transfer -h config/recordHandlers/JDBCXMLRecordHandler.xml -o config/jenaModels/VIVO.xml -O modelName=d2rStaging

# Execute Transfer to load "d2rStaging" JENA model into VIVO
java -cp bin/ingest-0.7.0.jar:bin/dependency/* org.vivoweb.ingest.transfer.Transfer -i config/jenaModels/VIVO.xml -I modelName=d2rStaging -o config/jenaModels/VIVO.xml

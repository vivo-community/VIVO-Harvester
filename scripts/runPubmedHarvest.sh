...
java -cp bin/ingest-$VERSION.jar:bin/dependency/* org.vivoweb.ingest.fetch.PubmedFetch -X config/tasks/PubmedFetch.xml
...
java -cp bin/ingest-$VERSION.jar:bin/dependency/* org.vivoweb.ingest.translate.XSLTranslator -i config/recordHandlers/Pubmed-XML-h2RH.xml -x config/datamaps/PubmedToVivo.xsl -o config/recordHandlers/Pubmed-RDF-h2RH.xml
...
java -cp bin/ingest-$VERSION.jar:bin/dependency/* org.vivoweb.ingest.transfer.Transfer -o config/jenaModels/h2.xml -O modelName=PubmedTempTransfer -O dbUrl="jdbc:h2:XMLVault/h2Pubmed/all/store;MODE=HSQLDB" -h config/recordHandlers/Pubmed-RDF-h2RH.xml

...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="rdf:Description">
		...
		<xsl:call-template name="t_Grant">
          ...
          <xsl:with-param name="grantid" select="concat($this, '/db-csv:GRANTID')" />
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template name="t_Grant">
		...
		<!-- Setting dates for Grant -->
	        <xsl:variable name="startDate">           
	            <xsl:analyze-string select="concat($this, '/db-csv:STARTDATE')" regex="^(....-..-..).*?$">
	                <xsl:matching-substring><xsl:value-of select="regex-group(1)"/></xsl:matching-substring>
	            </xsl:analyze-string>
	        </xsl:variable>

	        <xsl:variable name="endDate">           
	            <xsl:analyze-string select="concat($this, '/db-csv:ENDDATE')" regex="^(....-..-..).*?$">
	                <xsl:matching-substring><xsl:value-of select="regex-group(1)"/></xsl:matching-substring>
	            </xsl:analyze-string>
	        </xsl:variable>
...
<!--	Creating a Grant-->
		<rdf:Description rdf:about="{$baseURI}grant/grant{$grantid}">
            ...
            <xsl:if test="not( concat($this, '/db-csv:GRANTNAME') = '' or concat($this, '/db-csv:GRANTNAME') = 'null' )">
            <rdfs:label><xsl:value-of select="$this/db-csv:GRANTNAME"/></rdfs:label>
            </xsl:if>
            <xsl:if test="not( concat($this, '/db-csv:ADMINDEPARTMENTID') = '' or concat($this, '/db-csv:ADMINDEPARTMENTID') = 'null' )">
			<core:administeredBy>
<!--			Creating a department to match with or a stub if no match-->
				<rdf:Description rdf:about="{$baseURI}org/org{$this/db-csv:ADMINDEPARTMENTID}">
					<score:deptID><xsl:value-of select="concat($this, '/db-csv:ADMINDEPARTMENTID')"/></score:deptID>
					<rdfs:label><xsl:value-of select="concat($this, '/db-csv:ADMINDEPTNAME')"/></rdfs:label>
					<core:administers rdf:resource="{$baseURI}grant/grant{$grantid}" />
					<rdf:type rdf:resource="http://xmlns.com/foaf/0.1/Organization"/>
				</rdf:Description>
			</core:administeredBy>
			</xsl:if>
            <xsl:if test="not( concat($this, '/db-csv:AMOUNT') = '' or concat($this, '/db-csv:AMOUNT') = 'null' )">
			<core:totalAwardAmount><xsl:value-of select="concat($this, '/db-csv:AMOUNT')"/></core:totalAwardAmount>
			</xsl:if>
			
            <xsl:if test="not( concat($this, '/db-csv:SPONAWARDID') = '' or concat($this, '/db-csv:SPONAWARDID') = 'null' )">
            <core:sponsorAwardId><xsl:value-of select="$this/db-csv:SPONAWARDID"/></core:sponsorAwardId>
            </xsl:if>
			
			<xsl:choose>
				<xsl:when test="string(concat($this, '/db-csv:FLOWTHRUORG')) = '' or string(concat($this, '/db-csv:FLOWTHRUORG')) = 'null' "> 
                    <xsl:if test="not( concat($this, '/db-csv:AWARDINGORG') = '' or concat($this, '/db-csv:AWARDINGORG') = 'null' )">             
					<core:grantAwardedBy>
						<rdf:Description rdf:about="{$baseURI}sponsor/sponsor{$this/db-csv:AwardingOrgID}For{$grantid}">
							<rdfs:label><xsl:value-of select="concat($this, '/db-csv:AWARDINGORG')"/></rdfs:label>
                            <score:orgID><xsl:value-of select="concat($this, '/db-csv:AWARDINGORGID')"/></score:orgID>
				...
				<xsl:otherwise>
					...
					<core:grantAwardedBy>
						<rdf:Description rdf:about="{$baseURI}sponsor/sponsor{$this/db-csv:FLOWTHRUORGID}For{$grantid}">
							<rdfs:label><xsl:value-of select="concat($this, '/db-csv:FLOWTHRUORG')"/></rdfs:label>
                            <score:orgID><xsl:value-of select="concat($this, '/db-csv:FLOWTHRUORGID')"/></score:orgID>
							<core:awardsGrant rdf:resource="{$baseURI}grant/grant{$grantid}"/>
							<rdf:type rdf:resource="http://xmlns.com/foaf/0.1/Organization"/>
						</rdf:Description>
					</core:grantAwardedBy>
         ...
         <xsl:if test="not( concat($this, '/db-csv:STARTDATE') = '' or concat($this, '/db-csv:STARTDATE') = 'null' )">
         <rdf:Description rdf:about="{$baseURI}timeInterval/date{$startDate}">
             ...
             <core:dateTime rdf:datatype="http://www.w3.org/2001/XMLSchema#dateTime"><xsl:value-of select="concat($this, '/db-csv:STARTDATE')"/>T00:00:00</core:dateTime>
         </rdf:Description>
         </xsl:if>
                  
         <xsl:if test="not( concat($this, '/db-csv:ENDDATE') = '' or concat($this, '/db-csv:ENDDATE') = 'null' )">
         <rdf:Description rdf:about="{$baseURI}timeInterval/date{$endDate}">
             <rdf:type rdf:resource="http://www.w3.org/2002/07/owl#Thing"/>
             <rdf:type rdf:resource="http://vivoweb.org/ontology/core#DateTimeValue"/>
             <core:dateTimePrecision rdf:resource="http://vivoweb.org/ontology/core#yearMonthDayPrecision"/>
             <core:dateTime rdf:datatype="http://www.w3.org/2001/XMLSchema#dateTime"><xsl:value-of select="concat($this, '/db-csv:ENDDATE')"/>T00:00:00</core:dateTime>
         </rdf:Description>
         </xsl:if>
         
		<xsl:if test="not( concat($this, '/db-csv:PIID') = '' or concat($this, '/db-csv:PIID') = 'null' )">
		<!--            Creating the PI-->
				...
				<core:roleIn>
		         ...
		         <core:principalInvestigatorRoleOf>
		              <rdf:Description rdf:about="{$baseURI}person/person{$this/db-csv:PIID}">
							...
							<score:personID><xsl:value-of select="concat($this, '/db-csv:PIID')"/></score:personID>
							<xsl:if test="not( concat($this, '/db-csv:NAMEPI') = '' or concat($this, '/db-csv:NAMEPI') = 'null' )">
                                <rdfs:label><xsl:value-of select="concat($this, '/db-csv:NAMEPI')"/></rdfs:label>
                            </xsl:if>
		...
		<xsl:if test="not( concat($this, '/db-csv:COPIID') = '' or concat($this, '/db-csv:COPIID') = 'null' )">
		<!--            Creating the CoPI-->
			...
			<core:co-PrincipalInvestigatorRoleOf>
				<rdf:Description rdf:about="{$baseURI}person/person{$this/db-csv:COPIID}">
					<rdf:type rdf:resource="http://vivoweb.org/harvester/excludeEntity" />
					<score:personID><xsl:value-of select="concat($this, '/db-csv:COPIID')"/></score:personID>
                       <xsl:if test="not( concat($this, '/db-csv:NAMECOPI') = '' or concat($this, '/db-csv:NAMECOPI') = 'null' )">
                           <rdfs:label><xsl:value-of select="concat($this, '/db-csv:NAMECOPI')"/></rdfs:label>
                       </xsl:if>
					<rdf:type rdf:resource="http://xmlns.com/foaf/0.1/Person"/>
